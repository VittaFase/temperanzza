...
function BibliotecaHero() {
  ...
  return <section aria-labelledby="biblioteca-hero-title" className="relative min-h-[76svh] overflow-hidden bg-brand-ink text-white sm:min-h-[82svh]">
    {lead && <img src={lead.dish?.src ?? getProductImage(lead.featuredHandle) ?? ""} alt={lead.dish?.alt ?? lead.title} fetchPriority="high" decoding="async" className="absolute inset-0 h-full w-full object-contain p-12 sm:p-20" />}
...
function BibliotecaIndice() {
  const [aberta, setAberta] = useState<CategoriaKey | null>(null); const { refeicao, proteina, lifestyle, autor } = Route.useSearch(); const navigate = useNavigate({ from: "/cozinha" });
