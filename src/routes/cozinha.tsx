...
import { RECIPES, RECIPE_ASSET_QA, MOMENTS, PROTEINS, PROTEIN_ORDER, getRecipeProtein, type Recipe, type Moment } from "@/lib/recipes";
...
function BibliotecaHero() {
  ...
  return <section aria-labelledby="biblioteca-hero-title" data-qa-status={REBRAND_RECIPES.length ? "REBRAND_RECIPE_READY" : RECIPE_ASSET_QA.code} className="relative min-h-[76svh] overflow-hidden bg-brand-ink text-white sm:min-h-[82svh]">
    ...
    <div className="page-shell relative z-10 flex min-h-[76svh] items-end pb-14 pt-24 sm:min-h-[82svh] sm:pb-20">
      <div className="max-w-4xl"><span className="text-xs font-semibold uppercase tracking-[0.26em] text-white/70">Casa Temperanzza · Cozinha</span><h1 id="biblioteca-hero-title" className="mt-4 font-display text-[clamp(3.8rem,8vw,7.6rem)] font-semibold leading-[.86]">Cozinhe com mais sabor.</h1><p className="mt-6 max-w-2xl text-base leading-7 text-white/80 sm:text-lg">{REBRAND_RECIPES.length ? "Receitas reais, produto integrado à cena e inspiração para descobrir novas formas de usar os sabores da Casa." : "Nossa nova biblioteca gastronômica está recebendo os masters oficiais do Rebrand. Nenhuma imagem anterior será usada como substituição."}</p><div className="mt-8 flex flex-wrap items-center gap-5">{REBRAND_RECIPES.length ? <a href="#indice" className="inline-flex min-h-12 items-center gap-2 rounded-md bg-white px-6 py-3 text-sm font-semibold text-brand-ink transition hover:-translate-y-0.5">Explorar receitas <ChevronDown className="h-4 w-4" /></a> : <Link to="/produtos" className="inline-flex min-h-12 items-center gap-2 rounded-md bg-white px-6 py-3 text-sm font-semibold text-brand-ink transition hover:-translate-y-0.5">Conhecer os sabores <ArrowUpRight className="h-4 w-4" /></Link>}<span className="text-sm text-white/70"><CountUp to={REBRAND_RECIPES.length} from={Math.max(0, REBRAND_RECIPES.length - 3)} duration={1200} /> receitas publicadas</span></div>{lead && <Link to="/cozinha/$slug" params={{ slug: lead.slug }} search={{ refeicao: "", proteina: "", lifestyle: "", autor: "" }} className="mt-10 inline-flex items-center gap-2 border-b border-white/45 pb-1 text-sm font-semibold">Receita em destaque: {lead.title} <ArrowUpRight className="h-4 w-4" /></Link>}</div>
    </div>
  </section>;
}
