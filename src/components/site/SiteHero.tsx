...
export function SiteHero() {
  ...
  return (
    <section
      className="relative isolate w-full max-w-full overflow-hidden bg-brand-ink text-white"
      aria-label="Receitas em destaque"
    >
      <div className="relative w-full max-w-full overflow-hidden min-h-[70svh] sm:min-h-[76svh] lg:min-h-[calc(100svh-112px)]">
        {slides.map((recipe, index) => {
          ...
          return (
            <article
              key={recipe.slug}
              aria-hidden={!active}
              className={`absolute inset-0 w-full max-w-full overflow-hidden transition-opacity duration-700 motion-reduce:transition-none ${active ? "z-10 opacity-100" : "pointer-events-none z-0 opacity-0"}`}
            >
              <img
                src={recipe.dish!.src}
                alt={recipe.dish!.alt}
