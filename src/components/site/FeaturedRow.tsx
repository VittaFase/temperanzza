...
export function FeaturedRow({
  ...
  useEffect(() => {
    ...
    return () => { emblaApi.off("select", syncSelection); emblaApi.off("reInit", syncSelection); };
  ...
  return (
    <section aria-label={label} className="relative min-w-0 overflow-hidden py-4">
      <div className="mb-8 flex items-end justify-between gap-6 px-1 sm:mb-10">
        <div><span className="text-xs font-semibold uppercase tracking-[0.24em] text-muted-foreground">Casa Temperanzza</span><h2 className="mt-2 font-display text-4xl font-semibold leading-none text-brand-ink sm:text-5xl">{label}</h2></div>
      ...
      <div ref={emblaRef} className="w-full min-w-0 overflow-hidden touch-pan-y cursor-grab active:cursor-grabbing">
        <div className="flex -ml-3 sm:-ml-5">
          {products.map((p, index) => {
            ...
            const img = imageResolution.url;
            const tone = getFlavorTone(handle, p.node.title);
            const price = p.node.priceRange.minVariantPrice;
            const active = index === selectedIndex;

            return (
              <article key={p.node.id} data-featured-product={handle} data-image-source={imageResolution.source} className="min-w-0 flex-[0_0_84%] pl-3 sm:flex-[0_0_48%] sm:pl-5 lg:flex-[0_0_32%]" aria-current={active ? "true" : undefined}>
                <div className={`group relative flex min-h-[510px] flex-col overflow-hidden rounded-[2.25rem] p-6 transition-[transform,opacity] duration-500 ease-out sm:min-h-[570px] sm:p-8 ${active ? "scale-100 opacity-100" : "scale-[.94] opacity-70"}`} style={{ backgroundColor: tone.bg }}>
                  <Link to="/product/$handle" params={{ handle }} className="relative flex min-h-[310px] flex-1 items-center justify-center sm:min-h-[355px]" aria-label={`Ver ${p.node.title}`}>
                    {img ? <img decoding="async" src={img} alt={image?.altText || p.node.title} data-product-handle={handle} data-image-source={imageResolution.source} className={`h-[300px] w-[78%] object-contain drop-shadow-[0_22px_24px_rgba(0,0,0,.18)] transition-transform duration-500 ease-out sm:h-[350px] ${active ? "translate-y-0 scale-100" : "translate-y-3 scale-[.96]"} group-hover:-translate-y-1 group-hover:scale-[1.025]`} loading={index < 2 ? "eager" : "lazy"} /> : <div className="flex h-[280px] w-[70%] items-center justify-center rounded-[2rem] border border-brand-ink/10 bg-white/25 px-8 text-center text-sm text-brand-ink/55">Imagem oficial do produto</div>}
                  </Link>
                  <div className="relative z-10 mt-2 text-center">
                    ...
                    <div className="mt-5 flex items-center justify-center gap-2">
                      ...
                      <button type="button" onClick={() => handleAdd(p)} disabled={isLoading} className="inline-flex min-h-11 items-center gap-1.5 rounded-full bg-brand-ink px-5 py-2 text-xs font-semibold text-brand-paper transition hover:opacity-90 disabled:opacity-50">{isLoading ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Plus className="h-3.5 w-3.5" />} Sacola</button>
                    </div>
                  </div>
                </div>
              </article>
